...
Current Workflow Run: `resolve node → map input → execute node → persist output → choose edge → finish/pause`. Tool Nodes perform Policy checks. Uniform Policy and Sandbox enforcement across all nodes is the target mainline.
...
The current implementation uses JSON files under `state/*`, atomic renames, and per-ID serialized writes. This suits local and prototype deployment, but not multi-instance concurrency. Service extraction should replace Store implementations without changing domain Services.
