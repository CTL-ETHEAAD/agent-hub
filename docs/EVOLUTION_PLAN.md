...
实施状态：**进行中**（`feature/local-worker-runtime-v1`）。已新增 Worker Registry、心跳、能力标签、并发槽位、Node Run claim/lease/renew/complete 基础协议、过期 lease 回收和本地 CLI：`agent-hub worker` 与 `agent-hub scheduler`。当前 worker 支持可插拔 handler，并内置 start、condition、end 这类确定性节点处理；Agent/Tool 节点迁移到 Worker Handler 是下一步。
