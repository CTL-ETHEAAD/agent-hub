...
实施状态：**进行中**（`feature/delegation-governance-v1`）。已新增受治理的 Agent delegation service，Agent Run 支持 `rootRunId`、`parentRunId`、`depth` 和委派原因；委派会校验最大深度、每个父 run 的 child 数量和 allowed agent 清单，并提供 Run Tree 查询与树级取消入口。
...
实施状态：**进行中**（`feature/delegation-governance-v1`）。Worker Registry 已加入 capability attestation 字段；新增 governance snapshot 服务与 Admin API，可汇总 Agent/Workflow Run、Worker attestation、Trace 类型和基础 regression gate 结果，为后续数据库 Store、Queue/Event Bus、RBAC 和审计导出留出替换点。
