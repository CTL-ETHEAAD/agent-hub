...
test('validates a minimal workflow', () => {
  const workflow = validateWorkflow(valid);
  assert.equal(workflow.status, 'draft');
  assert.equal(workflow.contractVersion, 1);
});
test('rejects unsupported node contract versions', () => assert.throws(() => validateWorkflow({ ...valid, contractVersion: 2 }), /invalid/));
