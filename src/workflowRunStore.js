...
export async function createWorkflowRun({ workflow, input, idempotencyKey = '', parentRunId = '', spec = null }, root = WORKFLOW_RUNS_ROOT) {
  ...
  const run = {
    ...
    error: null,
    currentNodeId: null,
    nodes: Object.fromEntries(workflow.nodes.map((node) => [node.id, { id: node.id, type: node.type, status: 'pending', input: null, output: null, error: null, nodeRunId: null, agentRunId: null, startedAt: null, completedAt: null, durationMs: null }])),
    createdAt: now,
    startedAt: null,
    completedAt: null,
    durationMs: null,
