...
function renderAgents() {
  agentGrid.innerHTML = agents.length ? agents.map((agent) => `
    <article class="agent-card">
      ...
      <div class="agent-meta"><span>v${agent.version}</span><span>${escapeHtml(agent.runtime.provider)}</span><span>${escapeHtml(agent.permissions.filesystem)}</span>${agent.compatibility?.breaking ? `<span class="compatibility-breaking" title="${escapeHtml(agent.compatibility.changes.map((item) => item.message).join(' '))}">breaking contract</span>` : ''}</div>
      <footer>
...
function renderTools() {
  toolGrid.innerHTML = tools.length ? tools.map((tool) => `
    <article class="agent-card tool-card"><header><div class="agent-icon">T</div><span class="status-badge">${escapeHtml(tool.status)}</span></header><h2>${escapeHtml(tool.name)}</h2><p>${escapeHtml(tool.description || 'No description yet.')}</p><div class="agent-meta"><span>v${tool.version}</span><span>${escapeHtml(tool.type)}</span><span>${tool.type === 'http' ? escapeHtml(tool.config.method || 'GET') : escapeHtml(tool.config.server || 'MCP')}</span>${tool.compatibility?.breaking ? `<span class="compatibility-breaking" title="${escapeHtml(tool.compatibility.changes.map((item) => item.message).join(' '))}">breaking contract</span>` : ''}</div><footer>${tool.status === 'draft' ? `<button data-tool-action="edit" data-tool-id="${tool.id}">Edit</button><button data-tool-action="publish" data-tool-id="${tool.id}">Publish</button>` : `<button data-tool-action="new-version" data-tool-id="${tool.id}">New version</button>`}<button class="btn-primary" data-tool-action="test" data-tool-id="${tool.id}">Test</button><button data-tool-action="clone" data-tool-id="${tool.id}">Clone</button><button data-tool-action="archive" data-tool-id="${tool.id}">Archive</button></footer></article>`).join('') : '<div class="empty-agents"><h2>No tools yet</h2><p>Create an allowlisted HTTP Tool or register an MCP capability.</p></div>';
